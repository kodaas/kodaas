...
export const CustomMarkdownComponent: Components = {
  a({ children, href }) {
    return (
      <RefLink
        href={href!}
  ...
  ul: ({ children }) => <ul className="list-[square] mt-5 ml-5">{children}</ul>,

  ol: ({ children }) => <ol className="list-decimal mt-5 ml-5">{children}</ol>,
  ...
  blockquote: ({ children }) => (
    <blockquote className="relative overflow-hidden tracking-tight text-xs my-1 p-4 border-2 border-dashed dark:border-zinc-800 border-zinc-200 rounded-md">
      <BiSolidQuoteRight
        className="text-4xl -z-10 absolute -top-0 -right-2 -rotate-12 dark:text-zinc-700 text-zinc-300"
        aria-hidden="true"
      />
      {children}
    </blockquote>
  ...
  b: ({ children }) => (
    <b className="font-bold dark:text-zinc-300 text-zinc-700">{children}</b>
  ),
};
